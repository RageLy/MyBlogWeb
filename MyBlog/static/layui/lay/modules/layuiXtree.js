...
//获取全部选中的末级checkbox对象
layuiXtree.prototype.GetCheckedContainParent = function () {
    var _this = this;
    var arr = new Array();
    var arrIndex = 0;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked && (cks[i].getAttribute('data-xend') == '1' || cks[i].getAttribute('data-xend') == '0')) {
            arr[arrIndex] = cks[i];
            arrIndex++;
        }
    }
